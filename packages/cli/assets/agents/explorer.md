...
1. Lee `CLAUDE.md` para entender convenciones del repo — el motor global trae la orquestación; la ficha del repo (CLAUDE.md delgado, auto-cargada) y `argos.config.json` traen los hechos concretos (stack, skills aplicables) si los necesitas.
...
5. Escribe `.claude/progress/explore_<area>.md` (siempre relativo al repo donde trabajas):
...
Este archivo vive UNA vez en `~/.claude/agents/` y explora cualquier repo que
abras — no lo uses para hardcodear reglas de un repo puntual. Qué carpetas
excluir del scan, submódulos hermanos, o convenciones de naming propias de
CADA repo se resuelven en runtime desde su ficha (CLAUDE.md delgado) y su
`argos.config.json` (por ejemplo `stack` o `project.legacyPaths`).

Usa esta sección solo para convenciones de exploración que quieras aplicar a
TODOS tus repos por igual (ej: siempre excluir `dist/`, `*.gen.ts` y
artefactos de build de cualquier mapeo, sin importar el repo).
