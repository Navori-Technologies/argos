...
### 1. Arranque
Lee `CLAUDE.md` (reglas del proyecto + el bloque del orquestador), la ficha del repo (CLAUDE.md delgado, `argos:managed`) o su `argos.config.json`, y la `user-section` de abajo. Fija el scope: **targeted** (1 archivo/feature/módulo) o **full** (todo `src/`).
...
3. **Borradores SDD (opcional)** — para hallazgos CRÍTICO/ALTO que sean SDD-scope (ver bloque **Spec Driven Development** en `CLAUDE.md`), escribe `<specs-dir>/<feature>/{requirements,tasks}.md.draft`, donde `<specs-dir>` es el directorio de specs SDD del repo resuelto desde su ficha/`argos.config.json` (por defecto `specs/`). El `leader` los refina y les quita el `.draft`.
...
<!-- user: agrega aquí lo específico de tu stack. Sugerencias:
     ...
     - Áreas críticas que casi siempre requieren audit (declaradas en la ficha del repo o `argos.config.json`).
